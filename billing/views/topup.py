        ...
        if request.method == "POST":
            ...
            if request.POST.get("action") == "pay" and selected_payment_method_type:
                ...
                if selected_payment_method_type == "stripe_pm":
                    ...
                    account_gc_id = account.get_gocardless_id()
                    billing_params = {
                        "payment_request": {
                            "amount": amount_int,
                            "currency": "GBP",
                    ...
                    if num_bacs_mandates < 1:
                        billing_params["mandate_request"] = {
                            "currency": "GBP",
                        }
                    if account_gc_id:
                        billing_params["links"]["customer"] = account_gc_id
                    billing_request = gocardless_client.billing_requests.create(params=billing_params)
...
def complete_top_up_bank_transfer_stripe(request, item_id):
    ...
    if bank_instructions["type"] == "gb_bank_transfer":
